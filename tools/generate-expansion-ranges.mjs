...
export function renderSql(ranges, generatedAt = new Date()) {
  const localeRows = EXPANSIONS.map(
      (expansion) =>
          `    ('expansion', '${expansion.id}', 'name', '${escapeSql(expansion.name)}', '${escapeSql(expansion.name)}')`,
  ).join(",\n");

  const expansionRows = EXPANSIONS.map(
      (expansion) =>
          `    (${expansion.id}, '${escapeSql(expansion.slug)}', (SELECT id FROM locale WHERE source_type = 'expansion' AND source_key = '${expansion.id}' AND source_field = 'name'), ${expansion.majorVersion}, ${expansion.displayOrder})`,
  ).join(",\n");
...
INSERT INTO locale (source_type, source_key, source_field, en_us, en_gb)
VALUES
${localeRows}
