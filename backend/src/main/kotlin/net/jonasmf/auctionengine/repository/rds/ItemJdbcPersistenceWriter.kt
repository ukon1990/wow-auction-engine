...
private const val ITEM_PERSISTENCE_CHUNK_SIZE = 200

internal class ItemJdbcPersistenceWriter(
    ...
    private fun upsertLocales(records: List<LocaleRecord>) {
        ...
        records.chunked(ITEM_PERSISTENCE_CHUNK_SIZE).forEach { chunk ->
            val sql =
                buildString {
                    append(
                        """
    ...
    private fun findLocaleIds(keys: Collection<LocaleNaturalKey>): Map<LocaleNaturalKey, Long> {
        ...
        keys.chunked(ITEM_PERSISTENCE_CHUNK_SIZE).forEach { chunk ->
                ...
                { resultSet ->
                    val key =
                        LocaleNaturalKey(
                            ...
                            resultSet.getString("source_field"),
                        )
                    results[key] = resultSet.getLong("id")
                },
                *params.toTypedArray(),
            )
        }
    ...
    private fun upsertItemQualities(
        ...
        qualities.chunked(ITEM_PERSISTENCE_CHUNK_SIZE).forEach { chunk ->
            val sql =
                """
    ...
    private fun upsertInventoryTypes(
        ...
        inventoryTypes.chunked(ITEM_PERSISTENCE_CHUNK_SIZE).forEach { chunk ->
            val sql =
                """
    ...
    private fun upsertItemBindings(
        ...
        bindings.chunked(ITEM_PERSISTENCE_CHUNK_SIZE).forEach { chunk ->
            val sql =
                """
    ...
    private fun upsertItemClasses(
        ...
        itemClasses.chunked(ITEM_PERSISTENCE_CHUNK_SIZE).forEach { chunk ->
            val sql =
                """
    ...
    private fun upsertItemSubclasses(
        ...
        itemSubclasses.chunked(ITEM_PERSISTENCE_CHUNK_SIZE).forEach { chunk ->
            val sql =
                """
    ...
    private fun findItemSubclassIds(itemSubclasses: Collection<ItemSubclass>): Map<String, Long> {
        ...
        itemSubclasses
            .distinctBy { subclassKey(it.classId, it.subclassId) }
            .chunked(ITEM_PERSISTENCE_CHUNK_SIZE)
            .forEach { chunk ->
                ...
                jdbcTemplate.query(
                    ...
                    { resultSet ->
                        results[subclassKey(resultSet.getInt("class_id"), resultSet.getInt("subclass_id"))] =
    ...
    private fun upsertItemAppearanceReferences(appearanceReferences: List<ItemAppearanceReference>) {
        ...
        appearanceReferences.chunked(ITEM_PERSISTENCE_CHUNK_SIZE).forEach { chunk ->
            val sql =
                """
    ...
    private fun upsertItems(
        ...
        items.chunked(ITEM_PERSISTENCE_CHUNK_SIZE).forEach { chunk ->
            val sql =
                """
                INSERT INTO `item` (
                    id,
    ...
    private fun upsertItemAppearanceLinks(items: List<Item>) {
        ...
        links.chunked(ITEM_PERSISTENCE_CHUNK_SIZE).forEach { chunk ->
            val sql =
                """
    ...
    private fun findNaturalIds(
        ...
        if (keys.isEmpty()) return emptyMap()
        val results = linkedMapOf<String, Long>()
        keys.distinct().chunked(ITEM_PERSISTENCE_CHUNK_SIZE).forEach { chunk ->
            jdbcTemplate.query(
                "SELECT internal_id, $keyColumn FROM $tableName WHERE $keyColumn IN (${placeholders(chunk.size)})",
                { resultSet -> results[resultSet.getString(keyColumn)] = resultSet.getLong("internal_id") },
                *chunk.toTypedArray(),
            )
        }
        return results
