...
@Component({
  ...
  template: `
    @if (open()) {
      <div class="fixed inset-0 z-40 bg-black/50" aria-hidden="true" (click)="close()"></div>

      <div
        class="fixed top-16 right-0 z-50 flex h-[calc(100vh-4rem)] w-[min(28rem,90vw)] translate-x-0 flex-col border-l border-white/10 bg-slate-950/95 shadow-xl backdrop-blur-2xl transition-transform duration-200 ease-out"
        role="dialog"
        aria-modal="true"
      ...
      >
        <header class="flex items-center justify-between border-b border-white/10 px-5 py-4">
          <h2 [id]="resolvedTitleId()" class="font-cinzel text-xl font-bold text-primary-container">
            {{ title() }}
          </h2>
          <button
            #closeBtn
