...
import { CopyButtonComponent, PaginationComponent, PaginationState } from '@ui';
import { detectColumnSortTypes, sortSqlRows } from './admin-sql-result-sort';

const pageSizes = [25, 50, 100, 250] as const;

@Component({
  selector: 'app-admin-sql-result',
  imports: [CopyButtonComponent, PaginationComponent],
  template: `
    @if (result(); as result) {
      <section class="grid gap-3" aria-label="SQL result">
        ...
        @if (showEffectiveSql()) {
          <div class="grid gap-2">
            <div class="flex flex-wrap items-center justify-between gap-3">
              ...
              <ee-copy-button
                [text]="result.effectiveSql"
                label="Copy"
                ariaLabel="Copy effective SQL"
              />
            </div>
            <pre
        ...
        @if (jsonText(); as json) {
          <div class="grid gap-2">
            <div class="flex flex-wrap items-center justify-between gap-3">
              <p class="ee-label text-outline">JSON result</p>
              <ee-copy-button [text]="json" label="Copy" ariaLabel="Copy JSON result" />
            </div>
            <pre
                  ...
                  <tr>
                    @for (column of result.columns; track column; let i = $index) {
                      <th class="border-b border-white/10 px-3 py-2">
                        <button
                          type="button"
                          class="flex w-full items-center gap-1 text-left transition hover:text-on-surface focus:outline-none focus:ring-2 focus:ring-primary-container"
                          [attr.aria-label]="'Sort by ' + (column || 'Column ' + (i + 1))"
                          [attr.aria-sort]="sortAria(i)"
                          (click)="toggleSort(i)"
                        >
...
export class AdminSqlResultComponent {
  ...
  protected readonly columnSortTypes = computed(() => {
    const result = this.result();
    if (!result) {
      return [] as const;
    }
    return detectColumnSortTypes(result.columns, result.rows);
  });

  protected readonly sortedRows = computed(() => {
    const result = this.result();
    if (!result) {
      return [] as readonly (readonly (string | null)[])[];
    }
    const columnIndex = this.sortColumn();
    if (columnIndex === null) {
      return [...result.rows];
    }
    const type = this.columnSortTypes()[columnIndex] ?? 'string';
    return sortSqlRows(result.rows, columnIndex, type, this.sortDirection());
  });
  ...
  protected toggleSort(columnIndex: number): void {
  ...
  protected sortAria(columnIndex: number): 'ascending' | 'descending' | 'none' {
    if (this.sortColumn() !== columnIndex) {
      return 'none';
    }
