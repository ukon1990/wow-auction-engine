...
import { CopyButtonComponent } from '@ui';
import {
  AdminSqlService,
...
@Component({
  ...
  imports: [
    AdminSqlCodeEditorComponent,
    AdminTableBrowserComponent,
    CopyButtonComponent,
    FormsModule,
    AdminSqlResultComponent,
  ],
  template: `
    <section
            ...
            <div class="grid gap-4">
              <div class="grid gap-2">
                <div class="flex flex-wrap items-center justify-end gap-2">
                  <button
                    type="button"
                    class="h-8 rounded border border-white/10 px-3 ee-label font-semibold text-on-surface transition hover:bg-white/5 focus:outline-none focus:ring-2 focus:ring-primary-container disabled:cursor-not-allowed disabled:opacity-60"
                    [disabled]="!sql().trim()"
                    (click)="formatSql()"
                  >
                    Format
                  </button>
                  <ee-copy-button [text]="sql()" label="Copy" ariaLabel="Copy SQL" />
                </div>
                <app-admin-sql-code-editor
                  #codeEditor
                  [value]="sql()"
                  [enterToSubmit]="true"
                  (valueChange)="updateSql($event)"
                  (enterPressed)="execute(modeEnum.Query)"
                />
              </div>
                ...
                <div class="flex flex-wrap items-end gap-3">
                  ...
                  <div class="flex flex-wrap gap-3">
                    @if (activeSavedQueryId()) {
                      <button
                        type="button"
                        class="h-10 rounded-md bg-primary px-4 font-semibold text-on-primary transition hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-primary-container disabled:cursor-not-allowed disabled:opacity-60"
                        [disabled]="!sql().trim()"
                        (click)="saveQuery()"
                      >
                        Update
                    ...
                    <div class="grid max-h-48 gap-2 overflow-y-auto">
                      @for (query of savedQueries(); track query.id) {
                        <div
                          ...
                          [class.border-primary-container]="activeSavedQueryId() === query.id"
                          [class.bg-primary-container/10]="activeSavedQueryId() === query.id"
                          [class.border-white/10]="activeSavedQueryId() !== query.id"
                          [class.bg-surface-container]="activeSavedQueryId() !== query.id"
                        >
                          <button
                            type="button"
...
export class AdminSqlEditorComponent {
  ...
  private readonly codeEditor = viewChild(AdminSqlCodeEditorComponent);

  protected readonly open = signal(false);
  protected readonly sql = signal('');
  protected readonly saveName = signal('');
  protected readonly activeSavedQueryId = signal<string | null>(null);
  ...
  protected closeEditor(): void {
    this.open.set(false);
    this.activeSavedQueryId.set(null);
  }
  ...
  protected saveQuery(asNew = false): void {
    const sql = this.sql().trim();
    if (!sql) {
      return;
    }

    const activeId = this.activeSavedQueryId();
    if (!asNew && activeId) {
      const updated = this.savedQueryStorage.update(activeId, sql, this.saveName());
      if (updated) {
        this.saveNotice.set(`Updated "${updated.name}".`);
        return;
      }
      this.activeSavedQueryId.set(null);
    }
    ...
    this.activeSavedQueryId.set(saved.id);
    this.saveName.set(saved.name);
    this.saveNotice.set(`Saved "${saved.name}".`);
  }

  protected loadSavedQuery(query: SavedAdminSqlQuery): void {
    this.sql.set(query.sql);
    this.saveName.set(query.name);
    this.activeSavedQueryId.set(query.id);
    this.error.set(null);
    this.saveNotice.set(null);
  }
