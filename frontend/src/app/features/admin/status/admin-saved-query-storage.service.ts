...
export class AdminSavedQueryStorageService {
  ...
  update(id: string, sql: string, name: string): SavedAdminSqlQuery | null {
    const existing = this.queries().find((query) => query.id === id);
    if (!existing) {
      return null;
    }
    const query: SavedAdminSqlQuery = {
    ...
    this.write(this.queries().map((entry) => (entry.id === id ? query : entry)));
    return query;
  }
