...
if [[ -n "${WORKFLOW_RUN_ID:-}" ]]; then
  ...
  artifact_id="$(curl -fsSL \
    ...
    "$api_url" | python3 -c "import json,sys; data=json.load(sys.stdin); arts=[a for a in data['artifacts'] if a['name']=='backend-war' and not a['expired']]; print(arts[0]['id'] if arts else '')")"
...
if [[ "$war_ready" != "true" ]]; then
  ...
  (cd "$backend_dir" && ./mvnw -B -ntp "-Dkotlin.compiler.daemon=false" -DskipTests package)
  war_path="$(find "${backend_dir}/target" -maxdepth 1 -name '*.war' | head -n 1)"
  if [[ -z "$war_path" ]]; then
